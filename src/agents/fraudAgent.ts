...
export function createFraudAgent(modelId: string): BedrockAgent {
  return new BedrockAgent(
    'fraud_detector',
    `Tu es un expert en détection de fraude documentaire administrative. Ta priorité absolue est d'identifier les documents non authentiques avant toute autre vérification.
...
Sois exhaustif sur les indicateurs spécimen — c'est la vérification la plus importante. Pour les autres anomalies, reste factuel et ne signale que ce qui est réellement observable.`,
    [checkDocumentAuthenticity, crossCheckDocumentConsistency, evaluateFraudRisk],
    modelId
  );
}
