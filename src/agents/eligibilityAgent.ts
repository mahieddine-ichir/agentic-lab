...
export function createEligibilityAgent(modelId: string): BedrockAgent {
  return new BedrockAgent(
    'eligibility_checker',
    `Tu es un expert en règles d'éligibilité aux communautés.
...
- Si non éligible: les critères manquants`,
    [checkStudentEligibility, checkFamilleNombreuseEligibility, checkJeuneParentEligibility],
    modelId
  );
}
